...
async function initDB() {
  ...
  try { db.run("ALTER TABLE order_steps ADD COLUMN mfg_steps TEXT DEFAULT '[]'"); } catch(e) { /* column already exists */ }
...
function buildOrder(row) {
  ...
  return {
    ...
    plannedDates, doneDates, stepNotes, mfgSteps
  };
}
...
app.post('/api/orders', (req, res) => {
  try {
    ...
    run("INSERT INTO orders(id,customer,factory,product,quantity,customer_po,prepayment,start_date,due_date,notes) VALUES(?,?,?,?,?,?,?,?,?,?)",
      [id,customer||'',factory||'',product||'',quantity||'',customerPO||'',prepayment?1:0,startDate||'',dueDate||'',notes||'']);
    const steps = getSteps();
...
app.put('/api/orders/:id', (req, res) => {
  try {
    ...
    run("UPDATE orders SET customer=?,factory=?,product=?,quantity=?,customer_po=?,prepayment=?,start_date=?,due_date=?,notes=? WHERE id=?",
      [customer||'',factory||'',product||'',quantity||'',customerPO||'',prepayment?1:0,startDate||'',dueDate||'',notes||'',req.params.id]);
...
app.put('/api/config/suppliers', (req, res) => {
  try { run("INSERT OR REPLACE INTO config(key,value) VALUES('suppliers',?)",[JSON.stringify(req.body.suppliers)]); res.json({ok:true}); }
  catch(e) { res.status(500).json({error:e.message}); }
});
