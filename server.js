...
app.post('/api/orders', (req, res) => {
  try {
    ...
    run("INSERT INTO orders(id,customer,factory,product,quantity,customer_po,prepayment,start_date,due_date,notes,items) VALUES(?,?,?,?,?,?,?,?,?,?,?)",
      [id,customer||'',factory||'',product||'',quantity||'',customerPO||'',prepayment?1:0,startDate||'',dueDate||'',notes||'',itemsJson]);
    const steps = getSteps();
...
app.put('/api/orders/:id', (req, res) => {
  try {
    ...
    run("UPDATE orders SET customer=?,factory=?,product=?,quantity=?,customer_po=?,prepayment=?,start_date=?,due_date=?,notes=?,items=? WHERE id=?",
      [customer||'',factory||'',product||'',quantity||'',customerPO||'',prepayment?1:0,startDate||'',dueDate||'',notes||'',JSON.stringify(req.body.items||[]),req.params.id]);
